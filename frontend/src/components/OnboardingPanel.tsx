...
export default function OnboardingPanel({ profile, onProfileChange, onAnalyze }: OnboardingPanelProps) {
  ...
  const [candidateInputs, setCandidateInputs] = useState<Record<string, string>>(() => {
    ...
    for (const c of profile.candidates) init[c.id] = c.address;
    return init;
  });

  const ready = isProfileReady(profile);

  const handleWorkSelect = useCallback((result: GeocodingResult) => {
    onProfileChange({ ...profile, workplace: { address: result.displayName, lat: result.lat, lon: result.lon } });
  ...
  const handleCandidateSelect = useCallback((id: string, result: GeocodingResult) => {
    const updated = profile.candidates.map(c =>
      c.id === id ? { ...c, address: result.displayName, lat: result.lat, lon: result.lon } : c,
    );
    onProfileChange({ ...profile, candidates: updated });
    setCandidateInputs(prev => ({ ...prev, [id]: result.displayName }));
  }, [profile, onProfileChange]);
      ...
      <div style={{
        ...
        <Section label="Workplace" icon="🏢">
          <AddressInput
            ...
            value={workInput}
            resolved={profile.workplace ? { lat: profile.workplace.lat, lon: profile.workplace.lon } : null}
        ...
        <Section label="Candidate Homes" icon="🏠">
          {profile.candidates.map((c, i) => (
            <div key={c.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', marginBottom: 8 }}>
              <span style={{ fontSize: 12, color: '#888', width: 14, flexShrink: 0, marginTop: 10 }}>{i + 1}.</span>
              <AddressInput
                placeholder={`Home address ${i + 1}...`}
                value={candidateInputs[c.id] ?? c.address}
                resolved={c.lat !== 0 ? { lat: c.lat, lon: c.lon } : null}
                onChange={text => setCandidateInputs(prev => ({ ...prev, [c.id]: text }))}
                onSelect={result => handleCandidateSelect(c.id, result)}
              />
              <button
                onClick={() => removeCandidate(c.id)}
                style={{ ...btnSecondary, color: '#ef5350', borderColor: '#ef535044', marginTop: 2 }}
              >
                X
              </button>
            </div>
